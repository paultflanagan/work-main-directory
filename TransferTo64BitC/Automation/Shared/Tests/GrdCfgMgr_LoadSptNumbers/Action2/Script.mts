' HEADER
'------------------------------------------------------------------'
'    Description:  Import SPT Number - Range
'
...
Option Explicit
Print Now & " - ACTION START: Import_Range"
reporter.ReportNote "ACTION started - Import Range Files"
...
While LCase(Trim(strStep)) <> "end"
	...
	If Not ImportProvisionFileSAP(objWindow, BuildPath(ProjectWorkingPath, strFileName), strResult) Then
		...
		LogResult Environment("Results_File"), False, dtStartTime, Now(), "Import_Range", strReference, strDescription, strExpectedResult
	Else
		reporter.ReportEvent micPass, "Step " & strStep, "Succeeded: " & strDescription
		LogResult Environment("Results_File"), True, dtStartTime, Now(), "Import_Range", strReference, strDescription, strExpectedResult
	End If
...
reporter.ReportEvent micDone, "Import Range Files", "ACTION completed"
Print Now & " - ACTION END: Import_Range"

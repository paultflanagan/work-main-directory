' HEADER
'------------------------------------------------------------------'
'    Description:  Functional Test - Set key values for AI(01)AI(21) import
'
...
Option Explicit
Print Now & " - ACTION START: Setup_AI02AI21"
reporter.ReportNote "ACTION started - Duplicate Functional Test - Dual Format import;Setup AI02AI21"
...
If FindRowInDataGrid(objWindow.SwfTable("dtgrdProduct"), "Product Name", "Product FT-D", False)  > -1 Then	' exists
	...
	If Left(LCase(strValue), 1) <> "x" Then ' unhidden
		dlgConfig.SwfEdit("txtKeyValue").Set "x" & strValue						' hide key value (add prefix)
		dlgConfig.SwfButton("btnSave").Click		
	End If

	SelectRowInDataGrid dlgConfig.SwfTable("dtgConfigs"), "Format Name", "AI(01)+AI(21)", False
	strValue = dlgConfig.SwfEdit("txtKeyValue").GetROProperty("Text")
	If Left(LCase(strValue), 1) = "z" Then ' hidden
		dlgConfig.SwfEdit("txtKeyValue").Set Right(strValue, Len(strValue)-1)	' unhide key value (strip prefix)
		dlgConfig.SwfButton("btnSave").Click		
	End If
	
	LogResult Environment("Results_File"), True, dtStartTime, Now(), "FT_DualFormat_Driver", Null, "Change Key Values for AI(0a(AI(21) import", "Key Values changed"
...
reporter.ReportEvent micDone, "Duplicate Functional Test - Dual Format import;Setup AI02AI21", "ACTION completed"
Print Now & " - ACTION END: Setup_AI02AI21"

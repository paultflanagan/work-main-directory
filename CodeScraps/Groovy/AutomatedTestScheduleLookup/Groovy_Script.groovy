...
def hours = ["--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","--","?"]
//Is a placeholder for the background color chosen for each machine and stores it for short term memory while the for loop runs below in html
def backroundColor = ""
def date = ""
//The path of the desired target folder from which tests will be taken. If blank, no specification will be made.
def targetPath = TargetProjectDirectory
//How recent, in days, a test must be made in order for this script to account for it. If 0, no time restriction will be made.
def maxDaysSince = Integer.parseInt(RecencyWindow)

//Obtains each machine ("node"), assigns each a color
...
//Obtains each build from every Job from every node
Jenkins.instance.getAllItems(Job).each{
    // Take jobs located
    if (it.getFullName().startsWith(targetPath)) {
        def lastBuild=it.getLastBuild()
        //Only grab jobs with most recent builds from within maxDaysSince days ago, or all jobs if maxDaysSince == 0
        if (lastBuild != null) {
            if (maxDaysSince == 0 || ((System.currentTimeMillis() - lastBuild.getTime().getTime())/86400000 <= maxDaysSince)) {
                def jobBuilds=it.getBuilds()
                def build=null
                def buildNotFound = true
                jobBuilds.each { selectedBuild ->
                    if (buildNotFound) {
                        def triggerCause = selectedBuild.getCauses()
                ...
                if(build != null) {
                    //Only grab jobs with automatically triggered builds from within maxDaysSince days ago, or all jobs if maxDaysSince == 0
                    if (maxDaysSince == 0 || ((System.currentTimeMillis() - build.getTime().getTime())/86400000 <= maxDaysSince)) {
                        ...
                        //Begin process of creating a Test item for display
                        ...
                        if (TimeUnit.MILLISECONDS.toHours(currentStatus) != 0){
                            ...
                            //The builds with less than an hour in duration only have the time they start plus an hour for rounding
                            var = runTimeHours
                            hours[var-1] = var.toString()
...
//Defines a MarkupBuilder variable to begin constructing the data in html
